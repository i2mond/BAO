...
def random_crop(images, cropsize, fills):
    if isinstance(fills, int): fills = (fills,)
    ...
    for img, f in zip(images, fills):
        ...
        if isinstance(img, PIL.Image.Image):
            ...
            if len(img.shape) == 3:
                ...
                cont = np.ones((cropsize, cropsize), dtype=np.float32) * f
...
# def crf_inference_label(img, labels, t=10, n_labels=91, gt_prob=0.7):#voc=21, coco=91
#     import pydensecrf.densecrf as dcrf
...
def crf_inference_label(img, labels, t=10, n_labels=91, gt_prob=0.7): # voc=21, coco=91
    ...
    rgb_image = np.ascontiguousarray(np.copy(img))
    if len(rgb_image.shape) == 2:
        rgb_image = cv2.cvtColor(rgb_image, cv2.COLOR_GRAY2RGB)
    
    d.addPairwiseBilateral(sxy=50, srgb=5, rgbim=rgb_image, compat=10)
