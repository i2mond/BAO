...
class VOC12SegmentationDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, label_dir, voc12_root, crop_size, 
    ...
    def __getitem__(self, idx):
        ...
        if self.rescale:
            img, label = imutils_amn.random_scale((img, label), scale_range=self.rescale, order=(3, 0))

        if self.img_normal:
            img = self.img_normal(img)

        if self.hor_flip:
            img, label = imutils_amn.random_lr_flip((img, label))

        if self.crop_method == "random":
            img, label = imutils_amn.random_crop((img, label), self.crop_size, (0, 255))
        elif self.crop_method == 'none':
            img, label = img, label
        else:
            img = imutils_amn.top_left_crop(img, self.crop_size, 0)
            label = imutils_amn.top_left_crop(label, self.crop_size, 255)

        img = imutils_amn.HWC_to_CHW(img)
...
class VOC12AffDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, label_la_dir, label_ha_dir, voc12_root, cropsize, radius=5,
