...
class ConvTransBlock(nn.Module):
    ...
    def forward(self, x, x_t):
        ...
        x_t, attn_weight = self.trans_block(x_st + x_t)#self training
...
class Net(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        #Label conditioning
        self.label_enc = nn.Linear(20, 1024) # voc(20,1024) coco(90,1024)
        ...
        self.avgpooling = nn.AdaptiveAvgPool2d(1)###bns
        self.conv_cls_head = nn.Conv2d(int(256 * channel_ratio), self.num_classes, kernel_size=3, stride=1, padding=1)
        
        #generat AM
        self.classifier_x = nn.Conv2d(int(256 * channel_ratio), self.num_classes, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, method, x, label_cls):
        B, c, w, h = x.shape#batch size
        ...
        if method == 'bao':
            y = self.label_enc(label_cls.float()).unsqueeze(-1).unsqueeze(-1)
            x = x * y
            am_x = self.classifier_x(x)
            n, c, h, w = am_x.shape
            am_flat = am_x.flatten(start_dim=2)
            am = torch.einsum('nhw,ncw->nch', attn_weights_no_cls, am_flat).reshape([n, self.num_classes, h, w])
            am_bg = am[:,:1,:,:]
            
            cams = torch.einsum('nhw,ncw->nch', attn_weights_no_cls, conv_flat).reshape([n, self.num_classes, h, w])
            cams_fg = cams[:,1:,:,:]

            camsfg_ambg = torch.cat([am_bg, cams_fg], dim=1)
            return conv_logits, trans_logits, am, cams, camsfg_ambg
